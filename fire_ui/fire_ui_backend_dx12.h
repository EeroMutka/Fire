...
static UI_TextureID UI_DX12_CreateAtlas(int atlas_id, uint32_t width, uint32_t height) {
	...
	// Create texture view
	{
		...
		UI_DX12_STATE.device->CreateShaderResourceView(texture, &desc, m_srvHeap->GetCPUDescriptorHandleForHeapStart());
	}
...
static void UI_DX12_Init(UI_Backend* backend, ID3D12Device* device) {
...
static void UI_DX12_Draw(UI_Outputs* outputs, ID3D12GraphicsCommandList* command_list) {
	...
	for (int i = 0; i < 2; i++) {
		//if (UI_DX12_STATE.atlases_mapped[i].pData) {
		//	ID3D11Texture2D* staging = UI_DX12_STATE.atlases_staging[i];
		//	ID3D11Texture2D* dest = UI_DX12_STATE.atlases[i];
		//	UI_DX12_STATE.dc->Unmap(staging, 0);
		//
		//	UI_DX12_STATE.dc->CopyResource(dest, staging);
		//
		//	memset(&UI_DX12_STATE.atlases_mapped[i], 0, sizeof(UI_DX12_STATE.atlases_mapped[i]));
		//}
	}
